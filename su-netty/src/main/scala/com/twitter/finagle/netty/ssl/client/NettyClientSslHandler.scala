...
import com.twitter.finagle.netty.ssl.{Alpn, SslConnectHandler}
 ...
 */
private[netty] class NettyClientSslHandler(
    params: Stack.Params)
   ...
   * to use the default, for Netty 4, we replace it with the [[NettyClientEngineFactory]]
   * instead.
   */
  private[this] def selectEngineFactory(ch: Channel): SslClientEngineFactory = {
    ...
    if (engineFactory == defaultEngineFactory) NettyClientEngineFactory(ch.alloc())
    else engineFactory
  }
