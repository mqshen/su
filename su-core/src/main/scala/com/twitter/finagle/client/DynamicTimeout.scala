...
private[finagle] object DynamicTimeout {
  ...
  private[this] def timeoutFn(
    ...
    latencyCompensation: Duration): () => Duration = () => {
  ...
  def perRequestModule[Req, Rep]: Stackable[ServiceFactory[Req, Rep]] =
    new Stack.Module3[TimeoutFilter.Param, param.Timer, LatencyCompensation.Compensation, ServiceFactory[Req, Rep]] {
  ...
  private[client] def totalFilter(
    params: Stack.Params): Filter.TypeAgnostic = {
