...
private[finagle] class MethodBuilderRetry[Req, Rep] private[client] (
   ...
   */
  def forClassifier(
    classifier: ResponseClassifier): MethodBuilder[Req, Rep] =
  ...
  private[client] def filter(
    scopedStats: StatsReceiver): Filter.TypeAgnostic = {
...
private[client] object MethodBuilderRetry {
  ...
  private def shouldRetry[Req, Rep](
    classifier: ResponseClassifier): PartialFunction[(Req, Try[Rep]), Boolean] =
  ...
  private def policyForReqRep[Req, Rep](
    shouldRetry: PartialFunction[(Req, Try[Rep]), Boolean]): RetryPolicy[(Req, Try[Rep])] =
    RetryPolicy.tries(
  ...
  private def filteredPolicy[Req, Rep](
    retryPolicy: RetryPolicy[(Req, Try[Rep])]): RetryPolicy[(Req, Try[Rep])] =
