...
import com.twitter.finagle.thrift.transport.netty.NettyTransport
...
object Thrift
  ...
  object ThriftImpl {
    private[this] val UseNettyToggleId: String = "com.twitter.finagle.thrift.UseNetty4"
    private[this] val nettyToggle: Toggle[Int] = Toggles(UseNettyToggleId)
    private[this] def useNetty: Boolean = nettyToggle(ServerInfo().id.hashCode)

    val Netty: ThriftImpl = ThriftImpl(NettyTransport.Client, NettyTransport.Server)
