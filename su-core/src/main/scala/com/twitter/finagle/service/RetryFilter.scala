...
class RetryFilter[Req, Rep](
  ...
  def this(
    retryPolicy: RetryPolicy[(Req, Try[Rep])],
    timer: Timer,
    statsReceiver: StatsReceiver) = this(
    retryPolicy,
    timer,
    statsReceiver,
    RetryBudget())
...
final class RetryExceptionsFilter[Req, Rep](
  ...
  def this(
    retryPolicy: RetryPolicy[Try[Nothing]],
    timer: Timer,
    statsReceiver: StatsReceiver = NullStatsReceiver) = this(
    retryPolicy,
    timer,
    statsReceiver,
    RetryBudget())
...
object RetryExceptionsFilter {
  ...
  def typeAgnostic(
    retryPolicy: RetryPolicy[Try[Nothing]],
    timer: Timer,
    statsReceiver: StatsReceiver = NullStatsReceiver): TypeAgnostic = new TypeAgnostic {
