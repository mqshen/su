...
private[finagle] object TrafficDistributor {
   ...
   */
  type BalancerEndpoints[Req, Rep] = Var[Activity.State[Set[EndpointServiceFactory[Req, Rep]]]] with Updatable[Activity.State[Set[EndpointServiceFactory[Req, Rep]]]]
  ...
  private def safelyScanLeft[T, U](
    init: U,
    stream: Event[Activity.State[T]])(f: (U, T) => U): Event[Activity.State[U]] = {
...
private[finagle] class TrafficDistributor[Req, Rep](
  ...
  private[this] def weightEndpoints(
    addrs: Event[Activity.State[Set[Address]]]): Event[Activity.State[Set[WeightedFactory[Req, Rep]]]] = {
  ...
  private[this] def partition(
    endpoints: Event[Activity.State[Set[WeightedFactory[Req, Rep]]]]): Event[Activity.State[Iterable[WeightClass[Req, Rep]]]] = {
