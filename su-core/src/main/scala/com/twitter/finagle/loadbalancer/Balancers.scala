...
object Balancers {
  ...
  private def newScopedBal[Req, Rep](
    sr: StatsReceiver,
    lbType: String,
    bal: ServiceFactory[Req, Rep]): ServiceFactory[Req, Rep] =
  ...
  def p2c(
    ...
    rng: Rng = Rng.threadLocal): LoadBalancerFactory = new LoadBalancerFactory {
  ...
  def aperture(
    ...
    useDeterministicOrdering: Boolean = false): LoadBalancerFactory = new LoadBalancerFactory {
   ...
   */
  def roundRobin(
    maxEffort: Int = MaxEffort): LoadBalancerFactory = new LoadBalancerFactory {
