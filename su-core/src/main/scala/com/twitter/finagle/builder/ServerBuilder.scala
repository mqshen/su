...
object ServerBuilder {
  ...
  def safeBuild[Req, Rep](
    ...
    builder: Complete[Req, Rep]): Server =
...
class ServerBuilder[Req, Rep, HasCodec, HasBindTo, HasName] private[builder] (
  ...
  protected def copy[Req1, Rep1, HasCodec1, HasBindTo1, HasName1](
    ps: Stack.Params,
    newServer: Stack.Params => FinagleServer[Req1, Rep1]): ServerBuilder[Req1, Rep1, HasCodec1, HasBindTo1, HasName1] =
    new ServerBuilder(ps, newServer)

  private def _configured[P, HasCodec1, HasBindTo1, HasName1](
    param: P)(
      implicit stackParam: Stack.Param[P]): ServerBuilder[Req, Rep, HasCodec1, HasBindTo1, HasName1] =
   ...
   */
  def codec[Req1, Rep1](
    codec: Codec[Req1, Rep1]): ServerBuilder[Req1, Rep1, Yes, HasBindTo, HasName] =
   ...
   */
  def codec[Req1, Rep1](
    codecFactory: CodecFactory[Req1, Rep1]): ServerBuilder[Req1, Rep1, Yes, HasBindTo, HasName] =
   ...
   */
  def codec[Req1, Rep1](
    codecFactory: CodecFactory[Req1, Rep1]#Server): ServerBuilder[Req1, Rep1, Yes, HasBindTo, HasName] =
    stack({ ps =>
      ...
      val newStack = StackServer.newStack[Req1, Rep1].replace(
        StackServer.Role.preparer, (next: ServiceFactory[Req1, Rep1]) =>
          codec.prepareConnFactory(next, ps + Stats(stats.scope(label)))).replace(TraceInitializerFilter.role, codec.newTraceInitializer)

      case class Server(
          stack: Stack[ServiceFactory[Req1, Rep1]] = newStack,
          params: Stack.Params = ps) extends StdStackServer[Req1, Rep1, Server] {
        ...
        protected def newListener(): Listener[Any, Any] = null
  ...
  def stack[Req1, Rep1](
    mk: Stack.Params => FinagleServer[Req1, Rep1]): ServerBuilder[Req1, Rep1, Yes, HasBindTo, HasName] =
    copy(params, mk)
   ...
   */
  def stack[Req1, Rep1](
    server: StackBasedServer[Req1, Rep1]): ServerBuilder[Req1, Rep1, Yes, HasBindTo, HasName] = {
  ...
  def build(service: Service[Req, Rep])(
    implicit THE_BUILDER_IS_NOT_FULLY_SPECIFIED_SEE_ServerBuilder_DOCUMENTATION: ServerConfigEvidence[HasCodec, HasBindTo, HasName]): Server = build(ServiceFactory.const(service))

  @deprecated("Used for ABI compat", "5.0.1")
  final def build(service: Service[Req, Rep],
                  THE_BUILDER_IS_NOT_FULLY_SPECIFIED_SEE_ServerBuilder_DOCUMENTATION: ThisConfig =:= FullySpecifiedConfig): Server = build(ServiceFactory.const(service), THE_BUILDER_IS_NOT_FULLY_SPECIFIED_SEE_ServerBuilder_DOCUMENTATION)
  ...
  final def build(serviceFactory: () => Service[Req, Rep])(
    implicit THE_BUILDER_IS_NOT_FULLY_SPECIFIED_SEE_ServerBuilder_DOCUMENTATION: ThisConfig =:= FullySpecifiedConfig): Server = build((_: ClientConnection) => serviceFactory())(THE_BUILDER_IS_NOT_FULLY_SPECIFIED_SEE_ServerBuilder_DOCUMENTATION)
  ...
  final def build(serviceFactory: (ClientConnection) => Service[Req, Rep])(
    implicit THE_BUILDER_IS_NOT_FULLY_SPECIFIED_SEE_ServerBuilder_DOCUMENTATION: ThisConfig =:= FullySpecifiedConfig): Server = build(new ServiceFactory[Req, Rep] {
  ...
  def build(serviceFactory: ServiceFactory[Req, Rep])(
    implicit THE_BUILDER_IS_NOT_FULLY_SPECIFIED_SEE_ServerBuilder_DOCUMENTATION: ServerConfigEvidence[HasCodec, HasBindTo, HasName]): Server = {
  ...
  final def build(serviceFactory: ServiceFactory[Req, Rep],
                  THE_BUILDER_IS_NOT_FULLY_SPECIFIED_SEE_ServerBuilder_DOCUMENTATION: ThisConfig =:= FullySpecifiedConfig): Server = build(serviceFactory)(
