...
private class DynNameFactory[Req, Rep](
  ...
  private case class Pending(
    q: immutable.Queue[(ClientConnection, Promise[Service[Req, Rep]], Stopwatch.Elapsed)]) extends State
...
private[finagle] object NameTreeFactory {
  ...
  def apply[Key, Req, Rep](
    ...
    rng: Rng = Rng.threadLocal): ServiceFactory[Req, Rep] = {
    ...
    case class Weighted(
        drv: Drv,
        factories: Seq[ServiceFactory[Req, Rep]]) extends ServiceFactory[Req, Rep] {
